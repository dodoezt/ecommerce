  ...
  return (
    <div className="w-full h-auto flex xl:flex-row flex-col items-start justify-between pt-20">
      <div className="xl:w-3/5 w-full flex flex-col items-start justify-start px-3">
        ...
        <main className="-webkit-scroll-hidden w-full h-auto xl:max-h-[30rem] md:max-h-[40rem] max-h-[25rem] overflow-y-scroll p-3 flex flex-col justify-start items-center gap-5">
            {cartItems.length === 0 ? (
              <div className="w-auto flex flex-col items-center justify-start gap-2 absolute top-1/2 left-1/2 -translate-x-1/2 ">
                ...
                <button className="px-10 py-1 bg-[#FF4081] font-Poppins text-xl text-white font-semibold"
                onClick={() => navigate('/')}
                >Gass</button>
              </div>
            ) : (
              ...
              {cartItems.map((item) => (
                <div key={item.id} className="w-full p-5 flex items-center justify-start gap-3 bg-[#212121] rounded-lg relative">
                  ...
                  <div className="flex-col flex items-start flex-1 xl:gap-1 md:gap-1 gap-2">
                    <div className="flex xl:w-full md:w-full w-[80%] xl:overflow-visible md:overflow-visible overflow-hidden">
                    ...
                    <div className="w-full flex justify-between items-center gap-3 no-drag">
                      <div className="w-full flex items-center justify-start gap-3">
                        <div className="xl:w-1/3 md:w-1/3 w-1/2 flex items-center justify-start outline-none border-[#ffffff3a] border rounded-full xl:p-2 md:p-2 p-1 gap-1">
                          <button
                    ...
                    <div className="xl:hidden md:hidden flex gap-1 text-nowrap">
                      ...
                      <h1 className="text-white font-Poppins xl:text-xl md:text-xl text-base font-medium">{formatCurrency(countIndividualSub(item.harga, count[item.id]))}</h1>
                    </div>
                  </div>
                </div>
              ))}
