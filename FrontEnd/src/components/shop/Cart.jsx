...
const Cart = () => {
  ...
  const handleCheckout = async () => {
    try {
        ...
        await axios.post('http://cawop.h.filess.io:3307/api/checkout', payload);
  ...
  const updateCartItems = async (id, value) => {
    try {
      await axios.patch(`http://cawop.h.filess.io:3307/api/cart/${id}`, {
        jumlah: value,
      });
  ...
  const deleteCartItem = async (id) => {
    try {
      await axios.delete(`http://cawop.h.filess.io:3307/api/cart/${id}`)
